from get_pr_data import REPORT_PROGRESS
...
def main():
  ...
  has_next = first_resp['data']['repository']['pullRequests']['pageInfo']['hasNextPage']
  cursor = first_resp['data']['repository']['pullRequests']['edges'][-1]['cursor']
  print(f"Total PRs to process {total}.")
  rows = [
    ["state", "created_at", "merged", "extracted_at"]
  ]
  to_csv(first_resp, rows)
  while has_next:
    progress = round(len(rows)/total, 2) * 100
    print(f"Processed {progress}% of the total ...")
    result = paginated_query(owner, repo, cursor)
    has_next = result['data']['repository']['pullRequests']['pageInfo']['hasNextPage']
    to_csv(result, rows)
